...
local explosions = {} -- Click Explode Mod Ui Modification
local activeExplosives = {}
...
function tick(df)
	...
	deltaTick = df
	local itemsToRemove = {}
	if #activeExplosives ~= 0 and not GetBool("game.paused") then
		for i,v in ipairs (activeExplosives) do
			activeExplosives[i][3] = activeExplosives[i][3] - df
			if activeExplosives[i][3] <= 0 then
				--[[if activeExplosives[i][4] > 0 and i+1 <= #activeExplosives then
					activeExplosives[i+1][3] = activeExplosives[i+1][3] + activeExplosives[i][4]
					activeExplosives[i][4] = 0
				end]]
				Explosion(activeExplosives[i][1], activeExplosives[i][2], true)
				table.insert(itemsToRemove, i)
			end
		end
		for i,v in pairs(itemsToRemove) do
			table.remove(activeExplosives, v)
			--SetString("hud.hintinfo", "Removing Index")
		end
		--SetString("hud.notification", tostring(#activeExplosives))
	end
...
function draw()
	...
	function activate()
		if GetBool("options.clickExplode.DetonatorNotifs") then
			SetString("hud.notification", "Activating " .. #explosions .. " explosives.")
		end
		local temp = 0
		for i,v in ipairs (explosions) do
			if v[3] == 0 then
				Explosion(v[1], v[2], true)
			else
				--SetString("hud.notification", "Added Explosive to Queue")
				table.insert(activeExplosives, {v[1], v[2], v[3], v[4]})
			end
		end
	end
	...
	--SetString("hud.notification", "functioning")
	--options.clickExplode.DetonatorTimer
	--options.clickExplode.DetonatorInBetweenDelay
	local detonatorTool = GetInt("options.clickExplode.DetonatorTool")
	if UiIsMousePressed() and #explosions ~= 0 and GetBool("options.clickExplode.Detonator") and detonatorTool ~= 0 and GetString("game.player.tool") == Tools[detonatorTool] then
		activate()
		--SetString("hud.notification", "Doing")
		explosions = {}
	end
	plyTransform = GetPlayerTransform()
    local hit, dist = Raycast(plyTransform.pos, direction, maxDist)
    if hit and GetBool("options.clickExplode") then
     	local hitPos = TransformToParentPoint(plyTransform, Vec(0, 0, dist * -1))
      	local xpos = math.abs(plyTransform.pos[1]) - (hitPos[1])  
	 	local zpos = math.abs(plyTransform.pos[3]) - (hitPos[3])

	 	-- HeavisideStep(xpos,zpos)
  --     	SetString(
		-- "hud.notification",
		--  "New X "..
		--  math.floor(plyTransform.pos[1]- hitPos[1])..
		--  " Y"..
		--  math.floor(plyTransform.pos[2]- hitPos[2])..
		--  " Z"..
		--  math.floor(plyTransform.pos[3]- hitPos[3])..
 	-- 	 "\nOriginal X "..
		--  math.floor(hitPos[1])..
		--  " Y"..
		--  math.floor(hitPos[2])
      ...
      local unrestrictedTool = GetInt("options.clickExplode.ToolRestriction")
	  
      if ((UiIsMouseDown() and GetBool("options.clickExplode.Hold")) or UiIsMousePressed()) and (GetString("game.player.tool") == Tools[unrestrictedTool] or unrestrictedTool == 11) then
		local explosionSize = GetInt("options.clickExplode.ExplosionSize")
		explosionSize = (explosionSize/10) 
		if GetBool("options.clickExplode.NotifDisplay") and not GetBool("options.clickExplode.ClickFire") then
			SetString("hud.notification", "" .. explosionSize) --.. "Vector:" .. " X:" .. math.floor(direction[1]) .. "Y:" .. math.floor(direction[2]) .. " Z:" .. math.floor(direction[3]))
		end
		if not GetBool("options.clickExplode.Detonator") then
			-- if not GetBool("options.clickExplode.ClickFire") then
			-- 	slice()
			-- 	-- simpleSpread(hitPos, explosionSize)
			-- 	-- Explosion(hitPos, explosionSize, true)
			-- else
			-- 	SpawnFire(hitPos)
			-- end

			-- VARIABLES FOR BELOW FUNCTION - CAN BE GRABEBD FROM MENU
			local sliceDirection = 0
			local sliceRange =10
			local numSlices = 10
			...
			sliceTool(
				maxDist,
				sliceDirection,
				GetBool("options.clickExplode.ClickFire"),
				sliceRange,
				numSlices,
				explosionSize
				)
			...
			if GetBool("options.clickExplode.DetonatorNotifs") then
				SetString("hud.notification", "Explosion Added: " .. #explosions+1)
			end
			
			local explosiveTimer = GetInt("options.clickExplode.DetonatorTimer")
			local explosiveDelay = GetInt("options.clickExplode.DetonationDelay")
			--SetString("hud.notification", explosiveTimer .. " " .. explosiveDelay)
			table.insert(explosions, {hitPos, explosionSize, explosiveTimer, explosiveDelay})
		end
      end
    end
	-- END Click Explode Mod Ui Modification END
end

--[[
	basic standalone slice function - onyl does hoorizontal explosions
	]]
function slice()
	local sliceRange = 10
	local maxDist = 500
    local plyTransform = GetPlayerTransform()
    for i=1,sliceRange,1 do
    	local fwdPos = TransformToParentPoint(plyTransform, Vec((-(sliceRange/2)+i), 0, maxDist * -1))
    	local direction = VecSub(fwdPos, plyTransform.pos)
    	direction = VecNormalize(direction)
    	local hit, dist = Raycast(plyTransform.pos, direction, maxDist)
	    if hit and GetBool("options.clickExplode") then
	     	local hitPos = TransformToParentPoint(plyTransform, Vec((-(sliceRange/2)+i), 0, dist * -1))
	 		Explosion(hitPos, explosionSize,true)
	 	end
	 end
 return 0
...
--[[
	Simple tool to perform a directional slice based on player target view

	grabs player locationand raycasts a line from the player to the eventual target 
			- ideally used one a hit post has already been location

	@maxDist    	- Distance from player to scan for potential targets 
	@sliceDirection - Direction of cut (0 for along horizontal, 1 for vertical)
	@sliceType 		- Usage of explosions or fire (0 for explosion, 1 for fire)
	@sliceRange 	- Range from origin of click to extend from
	@numSlices  	- Number of "slices" to perform along the slice range 
	@explosionSize  - size of Explosion
								]]
function sliceTool(maxDist,sliceDirection,sliceType,sliceRange,numSlices,explosionSize)
	local xAxis = 0
	local yAxis = 0
    local plyTransform = GetPlayerTransform()
    if sliceDirection > 0 then
    	xAxis = 0
    	yAxis = 1
    else
    	xAxis = 1
    ...
    for i=1,sliceRange,(sliceRange / numSlices) do
    	local fwdPos = TransformToParentPoint(
											plyTransform,
											Vec(
												((-(sliceRange/2)+i)*xAxis),
												((-(sliceRange/2)+i)*yAxis),
												maxDist * -1)
    										)
    	local direction = VecSub(fwdPos, plyTransform.pos)
    	direction = VecNormalize(direction)
    	local hit, dist = Raycast(plyTransform.pos, direction, maxDist)
	    if hit and GetBool("options.clickExplode") then
	     	local hitPos = TransformToParentPoint(
	     										plyTransform, 
	     										Vec(
	     											((-(sliceRange/2)+i)*xAxis),
	     											((-(sliceRange/2)+i)*yAxis), 
...
function simpleSpread(hitPos,explosionSize)
	local plyTransform = GetPlayerTransform()
	-- SetString("hud.notification", "X "..math.floor(plyTransform.pos[1]).." Y"..math.floor(plyTransform.pos[2]).." Z"..math.floor(plyTransform.pos[3])) -- this prints the coordinates top center of the screen   
  
	x = math.abs(plyTransform.pos[1] - hitPos[1])  
	z = math.abs(plyTransform.pos[3] - hitPos[3])

	 local direct = HeavisideStep(x,z)
	-- local direct = 2
	for i=1,5,0.25 do
		rangeHit = hitPos
		rangeHit[direct] = rangeHit[direct] +(-2.5  +i) 
		Explosion(rangeHit, explosionSize, true)
...
-- simple step function to decide direction, if x larger number then work that plane, otherwise alternative plane. If equal then go vertical

function HeavisideStep(x,y)
	local h = 1
	local t = (x+y)/2
	if (math.floor(x)>math.floor(t)) then
		h = 1
	elseif (math.floor(x)<math.floor(t)) then
		h =3
	else
		h = 2
	end
